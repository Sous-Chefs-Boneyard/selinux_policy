...
property :dir, String, default: lazy { "#{Chef::Config[:file_cache_path]}/#{module_name}" } # content to work with. Defaults to autogenerated name in the Chef cache. Can be provided and pre-populated
# Content options:
property :content, String # provide a 'te' file directly. Optional
property :dir_source, String # Source directory for module source code. If specified, will use "remote_directory" on the directory specified as `directory`
...
action :fetch do
  ...
  raise 'dont specify both directory_source and content' if new_resource.dir_source && new_resource.content

  if new_resource.dir_source
    remote_directory new_resource.dir do
      source new_resource.dir_source
  ...
  if new_resource.content
    file "#{new_resource.dir}/#{new_resource.module_name}.te" do
...
action :compile do
  ...
  execute "semodule-compile-#{new_resource.module_name}" do
    ...
    not_if "#{make_command} -q", cwd: new_resource.dir # $? = 1 means make wants to execute http://www.gnu.org/software/make/manual/html_node/Running.html
    only_if { use_selinux(new_resource.allow_disabled) }
...
action :install do
  filename = "#{new_resource.dir}/#{new_resource.module_name}.pp"
