...
action :add do
  execute "selinux-port-#{new_resource.port}-add" do
    ...
    not_if "/usr/sbin/semanage port -l | grep -P '#{new_resource.protocol}\\s*(\\s\\d+,)*\\s*#{new_resource.port}(,|$)'>/dev/null"
    only_if {use_selinux}
  end
end
...
action :delete do
  execute "selinux-port-#{new_resource.port}-delete" do
    ...
    only_if "/usr/sbin/semanage port -l | grep -P '#{new_resource.secontext}\\s+#{new_resource.protocol}\\s*(\\s\\d+,)*\\s*#{new_resource.port}(,|$)'>/dev/null"
    only_if {use_selinux}
  end
end
...
action :addormodify do
  execute "selinux-port-#{new_resource.port}-addormodify" do
    command <<-EOT
    if /usr/sbin/semanage port -l | grep -P '#{new_resource.protocol}\\s*(\\s\\d+,)*\\s*#{new_resource.port}(,|$)'>/dev/null; then
    ...
    EOT
    not_if "/usr/sbin/semanage port -l | grep -P '#{new_resource.secontext}\\s+#{new_resource.protocol}\\s*(\\s\\d+,)*\\s*#{new_resource.port}(,|$)'>/dev/null"
    only_if {use_selinux}
  end
end
